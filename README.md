...
このコードはすべての形式検査を通過する。TypeScript は型エラーを出さない。ESLint も通す。テストはモックが正しく返すから通る。しかし「決済処理」という文脈で、この catch ブロックの意味は**完全に壊れている**。

> **語りえぬものを、数えられるものに変換する。私的言語を、公的尺度に置き換える。**

違和感とは、言語ゲームの境界で生じる意味の断裂だ。どのツールも、自分のゲームの外側に言及できない。だから**数値**が要る。`EHD = 0.30` は「try-catch の 70% がエラーを握り潰している」という客観的事実。誰が測定しても `0.30` になる。
