...
コードは言語である。そして言語の意味は、辞書に書かれた定義ではなく、**実際の使われ方** によって決まる。

`catch (error) { }` という構文は、どの言語仕様書にも違反していない。
TypeScript は型エラーを出さない。ESLint のデフォルトルールも通す。テストはモックが正しく振る舞うから通る。

しかし、この構文が「決済処理のエラーハンドリング」という **文脈** に置かれたとき、その意味は破綻する。
エラーが発生しても誰にも伝わらない。ユーザーは決済が成功したと思い込む。
コードは「動いている」が、その **使用** は壊れている。

**文法的に正しい文が、意味をなさないことがある。**
...
型定義は `{ name: string }` と宣言する。
APIは実際に `{ name: "太郎", nickname: "タロー" }` を返す。

型という **規則** は守られている — `name` は確かに `string` だ。
しかし規則は、それが実際にどう適用されるかまでは規定しない。
`nickname` というフィールドの存在を、型は語らない。語れない。

プログラマーは型を信頼し、`user.nickname` にアクセスする。`undefined` が返る。
画面には何も表示されない。エラーも出ない。ただ、空白がある。

**形式的な規則を遵守しても、その規則が現実のすべてを記述しているわけではない。**

### 語りえぬものは沈黙する — そしてバグになる

`new Date(2024, 1, 1)` は1月1日か？ — 答えは2月1日。
`"👨‍👩‍👧".length` は1か？ — 答えは8。
`0.1 + 0.2 === 0.3` か？ — 答えは `false`。

これらは言語仕様として「正しい」動作だ。ドキュメントに明記されている。
しかし、その仕様を **全て知っている** プログラマーは存在しない。

誰もが暗黙の前提を持っている。月は1から始まる。文字の長さは見た目の数。小数の足し算は正確。
これらの前提は **語られない** 。コードに書かれない。レビューでも指摘されない。
そして本番で、沈黙のまま壊れる。
...
graph TB
    ...
    style Gap fill:#4a1d1d,stroke:#ef4444,color:#fca5a5,stroke-width:3px
```

形式的検証が「正しい」と保証する世界と、コードが実際に使われる世界の **あいだ** に、違和感は存在する。

- ESLint は **文法** を検査する。意味は検査しない。
- TypeScript は **構造** を検査する。使用を検査しない。
- テストは **特定の入力に対する出力** を検査する。すべての状況を検査しない。
- レビュアーは **自分の知識** で検査する。知らないことは検査できない。

どのツールも、**自分が検査できる範囲の外側** に言及できない。
違和感はその外側にある。

### なぜ数値化するのか

「違和感がある」という直感は、チームで共有できない。
「なんかこの catch 怪しい」は、修正の優先度にならない。
...
**EHD = 0.30** は「try-catch の 70% がエラーを握り潰している」という客観的事実。
感覚ではなく数値なので、チームで共有でき、改善を追跡できる。

語りえぬものを、数えられるものに変換する。
それがこのツールの仕事である。

### 3つの問い — 違和感の分類

この乖離を体系的に検出するために、3つの問いを立てる。
